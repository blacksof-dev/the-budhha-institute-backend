  ...
  try {
    ...
    if (!type) {
      return res
        ...
        .json({
          status: "fail",
          message: "Type is required (Image/Video to upload)"
        });
    }
    ...
    if (req.file && req.file.path) {
      updatedData.thumbnailImg = req.file.path;
    }

    const data = await testimonialVideo.findByIdAndUpdate(id, updatedData, {
      new: true,
    });
